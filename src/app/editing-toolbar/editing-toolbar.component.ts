...
export class EditingToolbarComponent implements OnInit, OnDestroy {
  ...
  private subsToShowSymbolPanel: Subscription;
	private subsToSymbolPanelClosed: Subscription;
	...
	constructor(
		...
		iconRegistry.addSvgIcon(
			...
			sanitizer.bypassSecurityTrustResourceUrl(
				"assets/img/baseline-line-nodes-24px.svg",
			),
		);
		iconRegistry.addSvgIcon(
			"selectBoxFeature",
			sanitizer.bypassSecurityTrustResourceUrl(
				"assets/img/baseline-select-box2-24px.svg",
			),
		);
		iconRegistry.addSvgIcon(
			"add_poly",
			sanitizer.bypassSecurityTrustResourceUrl(
				"assets/img/add-poly-layer24px.svg",
			),
		);
		iconRegistry.addSvgIcon(
			"identify",
		...
		iconRegistry.addSvgIcon(
			"measureLine",
			sanitizer.bypassSecurityTrustResourceUrl(
				"assets/img/measure-straighten-24px.svg",
			),
		);
		iconRegistry.addSvgIcon(
			"measureArea",
		...
		iconRegistry.addSvgIcon(
			"add_location_attr",
			sanitizer.bypassSecurityTrustResourceUrl(
				"assets/img/add_location_attr-24px.svg",
			),
		);
		iconRegistry.addSvgIcon(
			"ratingQuietArea",
		...
		this.subsToShowEditToolbar =
			this.openLayersService.showEditToolbar$.subscribe(
				(data) => {
		...
		this.subsToGeomTypeEditing = this.openLayersService.layerEditing$.subscribe(
			(data: any) => {
				...
				if (data.layerGeom.toLowerCase() !== "multi") {
					//auto enable draw shape mode for single geom layers
					console.log(
						"auto enable draw " +
							data.layerGeom +
							" mode for layer " +
    ...
    //events used to disable symbol panel button if symbol panel is open from editing
		...
		this.subsToSymbolPanelClosed =
			this.openLayersService.symbolPanelClosed$.subscribe((data) => {
				if (data === true) {
	...
	ngOnDestroy() {
		...
		this.subsToShowSymbolPanel.unsubscribe();
		this.subsToSymbolPanelClosed.unsubscribe();
	}
